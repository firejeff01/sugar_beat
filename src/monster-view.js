...
export class MonsterView {
  constructor(scene){
    ...
    this.beam=new THREE.Mesh(new THREE.CylinderGeometry(1,.18,1,18,1,true),this.beamMaterial);this.root.add(this.beam);
    this.particles=[];
    for(let i=0;i<44;i++){
  ...
  update(event,time,visible=true){
    ...
    const start=new THREE.Vector3(event.monsterX-event.side*3.2,2.55,-event.p),end=new THREE.Vector3(event.x-event.side*event.width/2,1.2,-event.p),delta=end.clone().sub(start);
    this.beam.visible=active&&event.type!=='lightning';this.beamMaterial.color.setHex(spec.color);this.beam.position.copy(start.clone().add(end).multiplyScalar(.5));this.beam.scale.set(event.depth*.43,delta.length(),event.depth*.43);this.beam.quaternion.setFromUnitVectors(new THREE.Vector3(0,1,0),delta.clone().normalize());
    ...
    if(this.bolt.visible){const positions=this.boltGeometry.attributes.position;let n=0;
      for(let branch=0;branch<3;branch++)for(let j=0;j<12;j++)for(let edge=0;edge<2;edge++){
        const u=(j+edge)/12,point=start.clone().lerp(end,u);point.y+=Math.sin((j+edge)*3+Math.floor(time*18))*u*.8;point.z+=(branch-1)*1.6*u+Math.sin((j+edge)*8+Math.floor(time*18))*.3;positions.setXYZ(n++,point.x,point.y,point.z);
